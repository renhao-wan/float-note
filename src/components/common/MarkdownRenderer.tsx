...
async function loadLocalImage(path: string): Promise<string> {
  try {
    ...
    console.log(
      '[FLOATNOTE] Image loaded successfully, data length:',
      base64Data.length
    );
    return base64Data;
  } catch (error) {
...
function SafeImage({ src, alt }: { src?: string; alt?: string }) {
  ...
  if (!imageSrc) {
    return (
      <span className="text-muted-foreground text-sm italic">加载中...</span>
    );
  }
  ...
  return (
    ...
    >
      <ReactMarkdown
        ...
        components={{
          h1: ({ children }) => (
            <h1 className="text-2xl font-semibold mb-4 mt-6 first:mt-0 text-foreground">
              {children}
            </h1>
          ),
          h2: ({ children }) => (
          ...
          code: ({
            ...
            const isInline =
              !props.className || !props.className.includes('hljs');
            return isInline ? (
              <code
                className="bg-muted px-1.5 py-0.5 rounded-xl text-sm font-mono text-foreground"
                {...props}
              >
                {children}
              </code>
            ) : (
              <code className="block text-foreground" {...props}>
                {children}
              </code>
            );
          },
          ...
          section: ({ children, ...props }) => {
            // 检查是否为脚注部分
            const isFootnotes =
              (props as Record<string, unknown>)['data-footnotes'] === '';
            return isFootnotes ? (
              <section
          ...
          del: ({ children }) => (
            <del className="line-through text-muted-foreground opacity-70">
              {children}
            </del>
          ),
        }}
