...
export function NotesPanel({
  ...
  const handleDragEnd = useCallback(
    async (event: DragEndEvent) => {
      ...
      console.log('[FLOATNOTE] DragEnd event:', {
        activeId: active.id,
        overId: over?.id,
      });
  ...
  if (!sidebarVisible) {
    return <div className="w-0 h-full overflow-hidden" data-notes-sidebar />;
  }
  ...
  return (
    ...
    >
      <div
        className="h-full bg-card/80 border-r border-border/30 flex flex-col"
        data-notes-sidebar
      >
