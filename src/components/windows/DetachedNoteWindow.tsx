...
export function DetachedNoteWindow({ noteId }: DetachedNoteWindowProps) {
  ...
  useEffect(() => {
    ...
    const setupListeners = async () => {
      const unlistenNoteDeleted = await listen<string>(
        'note-deleted',
        (event) => {
          if (event.payload === noteId) {
            console.log(
              '[FLOATNOTE] Detached window received note-deleted event, closing window'
            );
            appWindow.close();
          }
        }
  ...
  useEffect(() => {
    console.log(
      '[DETACHED-WINDOW] Setting up keyboard event listeners for note:',
      noteId
    );
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (isPrimaryModifier(e) && e.key.toLowerCase() === 'w') {
        ...
        if (window.location.search.includes(`note=${noteId}`)) {
          ...
          console.warn(
            '[DETACHED-WINDOW] Window context mismatch, ignoring Primary+W'
          );
        }
      }
    };
  ...
  useEffect(() => {
    const setupConfigListener = async () => {
      ...
      const unlisten = await listen('config-updated', (event) => {
        console.log(
          '[DETACHED-WINDOW] Config updated event received:',
          event.payload
        );
      ...
      <CustomTitleBar
        ...
        stats={{
          ...
          lastSaved: saveStatus.lastSaved
            ? saveStatus.getRelativeTime() || undefined
            : undefined,
        }}
      />
                  ...
                  {saveStatus.isSaving ? (
                    ...
                    <>
                      <span
                        ...
                        style={{ fontSize: '10px' }}
                      >
                        {t('editor.saved')} {saveStatus.getRelativeTime()}
                      </span>
