...
async fn test_position_zero_bug_reproduction() {
    ...
    let notes = vec![
        create_test_note(
            "zero-note",
            "Note at Position Zero",
            "ZERO_CONTENT_UNIQUE",
            Some(0),
        ),
        create_test_note(
            "one-note",
            "Note at Position One",
            "ONE_CONTENT_DIFFERENT",
            Some(1),
        ),
        create_test_note(
            "two-note",
            "Note at Position Two",
            "TWO_CONTENT_ANOTHER",
            Some(2),
        ),
    ];
    ...
    assert_eq!(
        pos_0_note.content, "ZERO_CONTENT_UNIQUE",
        "Position 0 should have correct content"
    );
    assert_eq!(
        pos_0_note.title, "Note at Position Zero",
        "Position 0 should have correct title"
    );
    ...
    assert_eq!(
        sorted_notes[0].position,
        Some(0),
        "First note should be position 0"
    );
    assert_eq!(
        sorted_notes[0].content, "ZERO_CONTENT_UNIQUE",
    ...
    for (i, note) in sorted_notes.iter().enumerate() {
        ...
        refreshed_sorted.sort_by(|a, b| match (a.position, b.position) {
            (Some(pos_a), Some(pos_b)) => pos_a.cmp(&pos_b),
            (Some(_), None) => std::cmp::Ordering::Less,
            (None, Some(_)) => std::cmp::Ordering::Greater,
            (None, None) => std::cmp::Ordering::Equal,
        ...
        assert_eq!(
            refreshed_sorted[0].position,
            Some(0),
            "After selecting {}, position 0 should still be position 0",
            selected_note.title
        );
        assert_eq!(
            refreshed_sorted[0].content, "ZERO_CONTENT_UNIQUE",
            "After selecting {}, position 0 content should be unchanged",
            selected_note.title
        );
        assert_eq!(
            refreshed_sorted[0].id, "zero-note",
            "After selecting {}, position 0 ID should be unchanged",
            selected_note.title
        );

        log_info!(
            "SIMPLIFIED_TEST",
            "  ✅ Position 0 stable after selecting {}",
            selected_note.title
        );
    }
    ...
    log_info!(
        "SIMPLIFIED_TEST",
        "✅ Position 0 bug reproduction test passed"
    );
}
...
async fn test_position_zero_vs_none() {
    ...
    assert_eq!(
        sorted_notes[0].content, "ZERO_POS_CONTENT",
        "First note should have right content"
    );
    assert_eq!(
        sorted_notes[1].position, None,
        "Second note should have position None"
    );
    assert_eq!(
        sorted_notes[1].content, "NONE_POS_CONTENT",
        "Second note should have right content"
    );
