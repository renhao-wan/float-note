...
use crate::modules::{file_notes_storage::FileNotesStorage, file_storage::FileStorageManager};
...
mod position_zero_tests {
    ...
    async fn test_position_0_with_valid_note() {
        ...
        assert_eq!(
            notes_vec[0].position,
            Some(0),
            "Note at index 0 should have position 0"
        );
        assert_eq!(
            notes_vec[0].title, "First Note",
            "Note at index 0 should be 'First Note'"
        );
        assert_eq!(
            notes_vec[0].content, "Content of first note",
            "Position 0 note content should be correct"
        );
    ...
    async fn test_position_0_vs_none_handling() {
        log_info!(
            "POSITION_BUG_TEST",
            "🧪 Testing position 0 vs None handling"
        );
        ...
        assert_eq!(
            notes_vec[0].position,
            Some(0),
            "First note should have position 0"
        );
        assert_eq!(notes_vec[0].title, "Note with Position 0");
        assert_eq!(
            notes_vec[1].position, None,
            "Second note should have position None"
        );
    ...
    async fn test_position_conflict_at_zero() {
        log_info!(
            "POSITION_BUG_TEST",
            "🧪 Testing multiple notes with position 0 (conflict)"
        );
        ...
        let positions: Vec<Option<i32>> = loaded_notes.values().map(|n| n.position).collect();
    ...
    async fn test_selecting_different_notes_position_0_consistency() {
        log_info!(
            "POSITION_BUG_TEST",
            "🧪 Testing position 0 consistency when selecting different notes"
        );
        ...
        let notes = vec![
            create_test_note(
                "note-0",
                "Position Zero Note",
                "This should always be at position 0",
                Some(0),
            ),
            create_test_note(
                "note-1",
        ...
        let position_0_note = loaded_notes
            .values()
        ...
        for (i, (note_id, _)) in loaded_notes.iter().enumerate() {
            ...
            assert_eq!(
                current_position_0_note.position,
                Some(0),
                "After selecting {}, position 0 should still be position 0",
                selected_note.title
            );
            ...
            log_info!(
                "POSITION_BUG_TEST",
                "  ✅ Position 0 remains consistent: {}",
                current_position_0_note.title
            );
        }
    ...
    async fn test_concurrent_note_access_position_0() {
        log_info!(
            "POSITION_BUG_TEST",
            "🧪 Testing concurrent access to position 0 note"
        );
        ...
        let notes = vec![
            create_test_note(
                "concurrent-0",
                "Concurrent Note 0",
                "Concurrent content 0",
                Some(0),
            ),
            create_test_note(
                "concurrent-1",
                "Concurrent Note 1",
                "Concurrent content 1",
                Some(1),
            ),
            create_test_note(
                "concurrent-2",
                "Concurrent Note 2",
                "Concurrent content 2",
                Some(2),
            ),
        ];
        ...
        for (i, (thread_id, note)) in results.iter().enumerate() {
            assert_eq!(
                note.id, first_result.id,
                "Thread {} got different position 0 ID: {} vs {}",
                thread_id, note.id, first_result.id
            );
            assert_eq!(
                note.content, first_result.content,
                "Thread {} got different position 0 content",
                thread_id
            );
            log_info!(
                "POSITION_BUG_TEST",
                "Thread {} got consistent position 0: {}",
                thread_id,
                note.title
            );
        }
    ...
    async fn test_note_creation_position_assignment() {
        log_info!(
            "POSITION_BUG_TEST",
            "🧪 Testing position assignment for new notes"
        );
        ...
        let loaded_notes = storage.load_notes().await.unwrap();
        let max_position = loaded_notes
            .values()
            .filter_map(|n| n.position)
            .max()
            .unwrap_or(-1);

        let new_note = create_test_note(
            "new-note",
            "New Note",
            "Just created",
            Some(max_position + 1),
        ...
        log_info!(
            "POSITION_BUG_TEST",
            "✅ Note creation position assignment test passed"
        );
    }
}
...
mod race_condition_tests {
    ...
    async fn test_concurrent_note_updates_position_0() {
        log_info!(
            "POSITION_BUG_TEST",
            "🧪 Testing concurrent updates affecting position 0"
        );
        ...
        final_sorted.sort_by(|a, b| match (a.position, b.position) {
            (Some(pos_a), Some(pos_b)) => pos_a.cmp(&pos_b),
            (Some(_), None) => std::cmp::Ordering::Less,
            (None, Some(_)) => std::cmp::Ordering::Greater,
            (None, None) => std::cmp::Ordering::Equal,
        ...
        assert_eq!(
            final_sorted[0].position,
            Some(0),
            "Position 0 should remain at position 0"
        );
        assert_eq!(
            final_sorted[0].id, "race-0",
            "Position 0 should still be the same note ID"
        );

        log_info!(
            "POSITION_BUG_TEST",
            "Final position 0 content: '{}'",
            final_sorted[0].content
        );
        log_info!(
            "POSITION_BUG_TEST",
            "✅ Concurrent update race condition test passed"
        );
    }
    ...
    async fn test_position_0_content_corruption() {
        log_info!(
            "POSITION_BUG_TEST",
            "🧪 Testing for position 0 content corruption bug"
        );
        ...
        let notes = vec![
            create_test_note(
                "content-0",
                "Note Zero",
                "CONTENT_ZERO_UNIQUE_MARKER_12345",
                Some(0),
            ),
            create_test_note(
                "content-1",
                "Note One",
                "CONTENT_ONE_DIFFERENT_MARKER_67890",
                Some(1),
            ),
            create_test_note(
                "content-2",
                "Note Two",
                "CONTENT_TWO_ANOTHER_MARKER_ABCDE",
                Some(2),
            ),
        ];
        ...
        for round in 0..10 {
            log_info!(
                "POSITION_BUG_TEST",
                "Round {}: Simulating note selection switching",
                round
            );
            ...
            for (note_id, note) in &loaded_notes {
                ...
                if position_0_note.content != expected_position_0_content {
                    log_error!(
                        "POSITION_BUG_TEST",
                        "🚨 BUG DETECTED! Position 0 content corrupted!"
                    );
                    log_error!(
                        "POSITION_BUG_TEST",
                        "  Expected: '{}'",
                ...
                assert_eq!(
                    position_0_note.id, "content-0",
                    "Position 0 note ID changed after selecting {}",
                    note.title
                );
            }
        }
        ...
        log_info!(
            "POSITION_BUG_TEST",
            "✅ Position 0 content corruption test passed (no corruption detected)"
        );
    }
}
...
mod integration_tests {
    ...
    async fn test_frontend_command_sequence() {
        log_info!(
            "POSITION_BUG_TEST",
            "🧪 Testing frontend command sequence simulation"
        );
        ...
        log_info!(
            "POSITION_BUG_TEST",
            "Original position 0 content: '{}'",
            original_position_0_content
        );
        ...
        for i in 0..initial_notes_list.len() {
            ...
            log_info!(
                "POSITION_BUG_TEST",
                "Simulating user selecting note {} ({})",
                i,
                selected_note_id
            );
            ...
            assert_eq!(
                refreshed_notes_list[0].content, original_position_0_content,
                "Position 0 content changed after selecting note {} ({})",
                i, selected_note.title
            );
            assert_eq!(
                refreshed_notes_list[0].id, initial_notes_list[0].id,
                "Position 0 ID changed after selecting note {} ({})",
                i, selected_note.title
            );

            log_info!(
                "POSITION_BUG_TEST",
                "  ✅ Position 0 remained stable after selecting note {}",
                i
            );
        }
        ...
        log_info!(
            "POSITION_BUG_TEST",
            "✅ Frontend command sequence test passed"
        );
    }
