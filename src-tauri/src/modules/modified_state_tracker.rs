...
impl ModifiedStateTracker {
        ...
        match hashes.get(note_id) {
            Some(existing_hash) => {
                ...
                if changed {
                    log_warn!(
                        "MODIFIED_STATE",
                        "⚠️ Content change detected for note {}: old_hash={}, new_hash={}",
                        note_id,
                        &existing_hash[..8],
                        &new_hash[..8]
                    );
            ...
            None => {
                log_debug!(
                    "MODIFIED_STATE",
                    "Note {} has no previous hash, marking as changed",
                    note_id
                );
    ...
    pub async fn clear_modified(&self, note_id: &str) {
        ...
        if flags.remove(note_id).is_some() {
            log_info!(
                "MODIFIED_STATE",
                "✅ Cleared modified flag for note {} (saved)",
                note_id
            );
        }
    }
