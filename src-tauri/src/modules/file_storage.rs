...
impl FileStorageManager {
                ...
                match self.load_note_from_file(&path).await {
                    Ok(note) => {
                        ...
                        if notes.contains_key(&note.id) {
                            log_error!(
                                "FILE_STORAGE",
                                "🚨 Unexpected duplicate ID: {} in file {:?}. Skipping file.",
                                note.id,
                                path
                            );
                            continue;
        ...
        for (note_id, note) in notes.iter_mut() {
            let needs_fix = match note.position {
                Some(position) if position < 0 => {
                    log_error!(
                        "FILE_STORAGE",
                        "🚨 INVALID POSITION: Note {} has negative position {}",
                        note_id,
                        position
                    );
                    true
                ...
                Some(position) if position_counts.get(&position).unwrap_or(&0) > &1 => {
                    ...
                    log_error!(
                        "FILE_STORAGE",
                        "🚨 POSITION CONFLICT: Note {} has position {} shared with {} other notes",
                        note_id,
                        position,
                        count - 1
                    );
                ...
                None => {
                    ...
                    log_debug!(
                        "FILE_STORAGE",
                        "Note {} has no position (this is OK)",
                        note_id
                    );
                    false
                }
    ...
    pub async fn save_note(&self, note: &Note) -> Result<(), String> {
        ...
        log_info!(
            "FILE_STORAGE",
            "💾 Wrote note {} to disk: {:?} ({} bytes, content_hash={})",
            note.id,
            file_path,
            note.content.len(),
            &content_hash[..8]
    ...
    pub async fn load_workspace_state(&self) -> Result<WorkspaceState, String> {
        ...
        if !workspace_file.exists() {
            let default_state = WorkspaceState {
                notes_directory: self.notes_dir.to_string_lossy().to_string(),
                ..Default::default()
            };
    ...
    pub async fn save_window_state(
        ...
        workspace
            .window_states
            .insert(note_id.to_string(), window_state.clone());
    ...
    pub async fn migrate_from_json(&self, json_path: &Path) -> Result<(), String> {
        ...
        log_info!(
            "FILE_STORAGE",
            "Migrating notes from JSON file: {:?}",
            json_path
        );
        ...
        log_info!(
            "FILE_STORAGE",
            "Migration complete. Backed up original to {:?}",
            backup_path
        );
